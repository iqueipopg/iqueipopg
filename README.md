...
**[black-scholes-greeks](https://github.com/iqueipopg/black-scholes-greeks)**: Black-Scholes prices and Greeks for European calls and puts, checked against an independent Monte Carlo pricer that simulates GBM paths and never uses the closed form. On Hull's textbook example the closed-form call is 4.7594 and one million paths give 4.7580 with a standard error of 0.0050, a gap of 0.28 standard errors. All five Greeks agree within 1.3 standard errors, and the closed form reproduces the values printed in Hull. The tests also check that the Monte Carlo 95 percent intervals contain the exact price about 95 percent of the time, which catches a mis-scaled error bar that price checks alone would miss.
